...
- Dashboard 汇总磁盘、Pool、容量、Dataset 和 SMART 健康状态
- WebSocket 实时快照、数据时效提示和手动强制刷新
- 网页修改轮询、SSH 和登录配置，保存后重建 runtime
- 不保存配置即可执行 SSH 连通性测试
- `fixture` 演示模式和可选 SSH 失败回退
- Docker + Nginx + Uvicorn 单容器部署
...
- 轮询器使用 WebSocket 客户端感知的活跃/空闲模式切换——有浏览器查看时使用快速间隔并在连接瞬间强制全量刷新，无浏览器时使用可配置的空闲慢速间隔

## 下一步

- 继续补强定时快照编辑与策略可见性
- 继续补强 replace / RAID-Z expansion 的候选盘说明和审计信息
- 增加独立后台活动任务持续对账，避免依赖任务 API 查询触发更新
