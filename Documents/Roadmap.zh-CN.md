...
- 设置界面中可配置的空闲间隔
- **SMART 健康监控** — 自动 smartctl 轮询、磁盘表内置健康列、完整属性弹窗、ATA/NVMe 支持、非物理设备过滤
...
- 为后端已有的计划 PATCH 能力补齐前端编辑表单（标题、时间 pattern、递归和保留数量）
- 在界面中更清楚展示策略归属和元数据
- 增加“哪些快照属于哪个计划”的可见性
...
- 继续增强任务日志和操作审计信息
- 增加独立后台活动任务持续对账，不再依赖任务 API 查询触发
- 为 SMART 解析器、轮询和接口补齐自动化测试
